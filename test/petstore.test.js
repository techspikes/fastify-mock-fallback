...
describe('Petstore mock responses', () => {
  describe('matched requests', () => {
    describe('response example matching', () => {
      ...
      it('uses 200 response example for operations without request examples', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))
      ...
      it('uses the accepted 200 response media examples entry for operations without request examples', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))
        ...
        try {
          ...
          assert.equal(response.statusCode, 200)
          assert.equal(response.body, 'plain featured pet')
        } finally {
          await app.close()
        }
      })

      it('treats omitted Accept as */* and uses the first response media', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))

        try {
          const response = await app.inject({ method: 'GET', url: '/pets/featured' })
      ...
      it('prioritizes explicit x-request-match over same-name response examples', async () => {
        const app = await buildApp(fixturePath('petstore-response-media-matching'))
      ...
      it('selects x-request-match response media using Accept negotiation', async () => {
        const app = await buildApp(fixturePath('petstore-response-media-matching'))
      ...
      it('returns 501 for operations without request examples when 200 response has no example', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))

        try {
          const response = await app.inject({ method: 'DELETE', url: '/pets/cache' })
      ...
      it('ignores non-200 response examples for operations without request examples', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))

        try {
          const response = await app.inject({ method: 'GET', url: '/pets/redirect-info' })
      ...
      it('returns 501 for operations without request examples when 200 content has empty examples', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))

        try {
          const response = await app.inject({ method: 'GET', url: '/pets/empty-examples' })
      ...
      it('uses 200 response example before examples for operations without request examples', async () => {
        const app = await buildApp(fixturePath('petstore-parameterless-response'))
    ...
    describe('request sources', () => {
      ...
      it('selects same-name request body media examples using Content-Type', async () => {
        const app = await buildApp(fixturePath('petstore-request-body-content-type-matching'))
      ...
      it('requires all examples with the same name across path and query parameters', async () => {
        const app = await buildApp(fixturePath('petstore-core'))
      ...
      it('matches query parameter example', async () => {
        const app = await buildApp(fixturePath('petstore-query-parameter'))

        try {
          const response = await app.inject({ method: 'GET', url: '/pet/findByStatus?status=available' })
      ...
      it('matches header parameter example', async () => {
        const app = await buildApp(fixturePath('petstore-inventory-header'))
        ...
        try {
          ...
          assert.equal(response.statusCode, 200)
          assert.deepEqual(response.json(), { available: 7, pending: 2, sold: 1 })
        } finally {
          await app.close()
        }
      })

      it('matches cookie parameter example', async () => {
        const app = Fastify({ logger: false })
        ...
        // Cookie examples require @fastify/cookie so request.cookies is populated.
          await app.register(await import('@fastify/cookie').then(m => m.default))
          await app.register(fastifyMockFallback, {
            specification: fixturePath('petstore-user-logout-cookie'),
            enable: true,
          })
          await app.ready()
      ...
      it('ignores parameters that do not define examples', async () => {
        const app = await buildApp(fixturePath('petstore-user-login-partial-examples'))
  ...
  describe('registration and parsing', () => {
    ...
    it('registers operations without operationId', async () => {
      const app = await buildApp(fixturePath('petstore-operation-without-operation-id'))
    ...
    it('loads the core Petstore examples from a JSON specification', async () => {
      ...
      try {
        await app.register(fastifyMockFallback, {
          specification: jsonFixturePath('petstore-core'),
          enable: true,
        })
        await app.ready()
    ...
    it('resolves YAML anchors, aliases, local $ref, and relative file $ref entries before registering mocks', async () => {
      const app = await buildApp(fixturePath('petstore-dereferenced-examples'))
    ...
    it('registers parameterless operations without 200 examples as 501 routes next to mockable operations', async () => {
      const app = await buildApp(fixturePath('petstore-order-mixed-operations'))
    ...
    it('registers operations without usable examples as 501 routes while keeping valid operations', async () => {
      const app = await buildApp(fixturePath('petstore-operations-without-examples'))

      try {
        // Incomplete operations return 501, while the complete one still works.
    ...
    it('handles a specification without paths', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-no-paths'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('handles a specification with empty paths', async () => {
      const app = await buildApp(fixturePath('petstore-empty-paths'))
  ...
  describe('unmatched requests', () => {
    ...
    it('returns 501 when cookie examples cannot be read without @fastify/cookie', async () => {
      const app = await buildApp(fixturePath('petstore-user-logout-cookie'))
      ...
      try {
        ...
        // Without @fastify/cookie, the raw cookie header is not enough to match.
        assert.equal(response.statusCode, 501)
        assert.equal(response.headers['x-mock-response'], 'true')
      } finally {
        await app.close()
  ...
  describe('registration validation', () => {
    it('throws during registration when an operation does not define response codes', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-order-with-missing-responses'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws for unsupported OpenAPI parameter locations', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-unsupported-parameter-location'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('registers no route when response examples are not linked to request examples for parameterized operations', async () => {
      const app = await buildApp(fixturePath('petstore-inventory-without-request-match'))
    ...
    it('throws during registration when response examples reference unavailable request body examples', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-missing-request-body-example'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws during registration when x-request-match references a missing request example', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-order-orphan-request-match'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws when x-request-match is defined outside response example level', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-request-match-invalid-location'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws when x-request-match is defined at response object level', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-request-match-invalid-response-location'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws when x-request-match is defined at response media level', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-request-match-invalid-media-location'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws when a $ref target does not exist', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-ref-missing-target'),
              enable: true,
            })
            await app.ready()
          },
    ...
    it('throws when $ref entries are circular', async () => {
      ...
      try {
        await assert.rejects(
          async () => {
            await app.register(fastifyMockFallback, {
              specification: fixturePath('petstore-ref-circular'),
              enable: true,
            })
            await app.ready()
          },
